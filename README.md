...
| On a harder autoregressive retrieval trace, matched 20,000-step entmax models reached 100% exact accuracy through sampled length 5,000, both with and without a final KEEP/SWAP target. With a fixed external bubble-sort controller, the action model then sorted all 16 sampled length-100 lists, with every one of the 79,200 local traces correct. | Preliminary | [KEEP/SWAP comparison](experiments/sparse_attention_adam/README.md#keepswap-extension), [sorting composition](experiments/sparse_attention_adam/README.md#full-bubble-sort-composition), [trace comparison JSON](experiments/sparse_attention_adam/results/pointer_compare_summary.json), [sorting JSON](experiments/sparse_attention_adam/results/pointer_trace_bubble_sort_summary.json) |
