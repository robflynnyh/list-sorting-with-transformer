...
def test_pointer_pair_prompt_targets_marked_and_following_value() -> None:
    vocabulary = PointerNextVocabulary()
    example = vocabulary.encode_pair_example_with_pointer([7, 4, 2], 1)

    assert vocabulary.render_tokens(example) == "<bos>7,<PTR>4,2=4,2<eos>"
    assert example[-4:] == [
        vocabulary.value_token(4),
        COMMA,
...
def test_pointer_pair_batch_requires_following_value() -> None:
    vocabulary = PointerNextVocabulary()
    ...
    assert batch.token_ids.shape == (64, 16)
    assert batch.model_inputs.shape == (64, 15)
    assert batch.prompt_ids.shape == (64, 12)
    assert torch.all(batch.pointers.ge(0))
    assert torch.all(batch.pointers.lt(4))
    assert int(batch.pointers.max()) == 3
    assert torch.all(batch.labels[:, : batch.prompt_length - 1].eq(IGNORE_INDEX))
    assert torch.all(batch.labels[:, batch.prompt_length - 1].ne(IGNORE_INDEX))
    ...
    for row, pointer, tokens in zip(
        ...
        assert tokens == vocabulary.encode_pair_example_with_pointer(row, pointer)
...
def test_pointer_pair_metrics_score_both_autoregressive_values() -> None:
    vocabulary = PointerNextVocabulary()
    values = torch.tensor([[7, 4, 2], [1, 3, 9]])
    pointers = torch.tensor([1, 0])
    generated = torch.tensor(
        [
            [vocabulary.value_token(4), COMMA, vocabulary.value_token(2), EOS],
            [vocabulary.value_token(1), COMMA, vocabulary.value_token(8), EOS],
        ]
    )

    metrics = generated_pointer_pair_metrics(
        values,
        pointers,
        generated,
        vocabulary,
        train_max_pointer_index=0,
    )

    assert metrics["exact_match"] == 0.5
    assert metrics["first_value_accuracy"] == 1.0
    assert metrics["second_value_accuracy"] == 0.5
    assert metrics["comma_accuracy"] == 1.0
    assert metrics["eos_accuracy"] == 1.0
    assert metrics["seen_pointer_exact_match"] == 0.0
    assert metrics["unseen_pointer_exact_match"] == 1.0
