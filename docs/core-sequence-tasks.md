...
[full report](rasp_transfer_report.md) for multi-seed tables, learning
...
also finds no length-generalization advantage, including when the compiled
layers are frozen throughout training. The
[language-model transfer report](language_model_transfer_report.md)
...
See the [metrics reference](metrics.md) for every training and evaluation
