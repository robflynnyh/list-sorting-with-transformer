...
| Metric | Meaning |
| --- | --- |
| `train/loss` | Mean teacher-forced cross-entropy over predicted outputs. Inputs and padding are excluded. Executor observations are excluded for `pointer_quicksort` and included for `pointer_quicksort_no_tool`. Lower is better. |
...
| Metric | Meaning |
| --- | --- |
| `exact_match` | The final answer exactly equals the correctly sorted input. Executor-assisted pointer runs must reach a valid `DONE`; no-tool pointer runs must also generate every observation correctly. |
...
| `target_token_accuracy` | Positional token accuracy of the freely generated final answer, or positional action accuracy for either pointer task. |
| `full_target_token_accuracy` | Positional accuracy over the complete generated target. It equals action accuracy for executor-assisted pointer runs and includes observations for no-tool runs. |
| `execution_completed` | Offline or interactive pointer executor reached `DONE` without an invalid action. Emitted for both pointer tasks. |
| `observation_token_accuracy` | Positional accuracy of observations generated by `pointer_quicksort_no_tool`. Missing observations after an early failure count as incorrect. |
| `observation_exact_match` | Every generated observation was correct and action replay reached `DONE`. Emitted for `pointer_quicksort_no_tool`. |
| `timed_out` | Pointer decoding did not produce `DONE` within its maximum generation budget. |
...
learned. For `pointer_quicksort`, only model actions are scored and gold
executor observations remain in context. For `pointer_quicksort_no_tool`,
actions and observations are both scored. These metrics do not show whether
the model can execute a complete trace autonomously, because free generation
can compound an early error.
