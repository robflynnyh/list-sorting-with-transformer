...
| `trace_exact_match` | Every generated quicksort event, machine action, and model-assigned observation or window exactly matches the deterministic reference trace. Fully executor-assisted traces contain actions only. |
...
| `window_token_accuracy` | Positional accuracy across fixed-width local windows assigned to the model. Required windows not reached after an early action failure count as incorrect. |
| `window_transition_exact_fraction` | Fraction of model-generated windows that exactly match the executor window. |
| `window_exact_match` | Every local window assigned to the model was generated exactly. |
