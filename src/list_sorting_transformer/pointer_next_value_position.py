"""Predict the position of the list value following a retrieved marked value."""
...
from .pointer_value_from_position import (
...
class ModularNextValuePositionModel(ModularPositionValueModel):
    """Generate the Stage 3 trace, then the address of the following value."""

    def target_next_value_position(
        self,
        pointers: Tensor,
        offsets: Tensor,
    ) -> Tensor:
        next_value_positions = offsets + 1 + 2 * pointers + 3
        return torch.stack(
            [
                next_value_positions.remainder(modulus)
                for modulus in self.moduli
            ],
            dim=-1,
        )

    def mixed_hidden_states(
        self,
        prompt_ids: Tensor,
        ...
        *,
        ...
        content_embeddings = torch.cat(
            (prompt_content, address_content, token_content),
            dim=1,
        )
    ...
    def generate_stage_four_trace(
        ...
        *,
        ...
        token_hidden = self.hidden_states(
            prompt_ids,
            positions,
            offsets=offsets,
        )
        tokens = self.token_logits(token_hidden[:, -1]).argmax(dim=-1)
        next_hidden = self.mixed_hidden_states(
            prompt_ids,
            positions,
            tokens[:, None],
            offsets=offsets,
        )
        next_logits = self.position_logits(next_hidden[:, -1])
        next_positions = torch.stack(
            [component.argmax(dim=-1) for component in next_logits],
            dim=-1,
        )
        return positions, tokens, next_positions


def load_stage_three_checkpoint(
    model: ModularNextValuePositionModel,
    checkpoint_path: Path,
) -> dict[str, int]:
    checkpoint = torch.load(checkpoint_path, map_location="cpu")
    if checkpoint.get("probe") != "pointer_value_from_position":
        raise ValueError("checkpoint is not a Stage-3 pointer-value model")
    source_state = checkpoint.get("model_state")
    if not isinstance(source_state, dict):
        raise ValueError("stage-three checkpoint is missing model_state")
    missing, unexpected = model.load_state_dict(source_state, strict=True)
    if missing or unexpected:
        raise ValueError(
            "stage-three architecture mismatch: "
            f"missing={list(missing)}, unexpected={list(unexpected)}"
        )
    return {
        "stage_three_step": int(checkpoint.get("step", 0)),
...
def generated_stage_four_metrics(
    generated_positions: Tensor,
    generated_tokens: Tensor,
    generated_next_position: Tensor,
    target_positions: Tensor,
    target_tokens: Tensor,
    target_next_position: Tensor,
    *,
    moduli: tuple[int, ...],
) -> dict[str, float]:
    ...
    stage_three_correct = (
        ...
        & generated_tokens.eq(target_tokens)
    )
    next_correct = generated_next_position.eq(target_next_position)
    next_exact = next_correct.all(dim=1)
    all_positions_exact = (
        generated_positions.eq(target_positions).flatten(1).all(dim=1)
        & next_exact
    )
    expected_next = torch.stack(
        [
            (generated_positions[:, 1, index] + 2).remainder(modulus)
            for index, modulus in enumerate(moduli)
        ],
        dim=-1,
    )
    next_consistent = generated_next_position.eq(expected_next).all(dim=1)
    stage_three_count = int(stage_three_correct.sum().item())
    metrics.update(
        {
            "stage_three_complete_trace_accuracy": metrics[
                "complete_trace_accuracy"
            ],
            "next_value_position_accuracy": float(
                next_exact.float().mean().item()
            ),
            "next_value_position_residue_accuracy": float(
                next_correct.float().mean().item()
            ),
            "next_value_position_consistency": float(
                next_consistent.float().mean().item()
            ),
            "all_positions_accuracy": float(
                all_positions_exact.float().mean().item()
            ),
            "complete_trace_accuracy": float(
                (stage_three_correct & next_exact).float().mean().item()
            ),
            "next_value_position_accuracy_given_stage_three_correct": (
                float(
                    (stage_three_correct & next_exact).sum().item()
                    / stage_three_count
                )
                if stage_three_count
                else 0.0
            ),
        }
    )
    ...
    *,
    ...
    for length in lengths:
        ...
        while processed < config.eval_examples:
            ...
            position_targets = model.target_sequence(batch.pointers, offsets)
            token_targets = target_token_ids(batch)
            next_targets = model.target_next_value_position(
                batch.pointers,
                offsets,
            )
            with autocast_context(device):
                generated = model.generate_stage_four_trace(
                    batch.prompt_ids,
                    offsets=offsets,
                )
                _, teacher_forced = next_value_position_loss_and_metrics(
                    model,
                    batch,
                    offsets,
                    config=config,
                ...
                json.dumps(
                    {
                        ...
                        "evaluation_next_value_position_accuracy": {
                            str(length): metrics[
                                "next_value_position_accuracy"
                            ]
                            for length, metrics in per_length.items()
                        },
                    }
...
def main() -> None:
    ...
    checkpoint = torch.load(args.stage_three_checkpoint, map_location="cpu")
    ...
    if not isinstance(checkpoint_model_config, dict):
        raise ValueError("stage-three checkpoint is missing model_config")
    if not isinstance(checkpoint_train_config, dict):
        raise ValueError("stage-three checkpoint is missing train_config")
    ...
    if model_config.vocab_size != vocabulary.size:
        raise ValueError("stage-three checkpoint vocabulary does not match")
    torch.manual_seed(config.seed)
    model = ModularNextValuePositionModel(
        model_config,
    ...
    initialization = load_stage_three_checkpoint(
        model,
    ...
    metadata = {
        ...
        "stage_three_checkpoint": str(args.stage_three_checkpoint),
        **initialization,
    }
